"""Product-analytics scorers for the sandboxed coding agent.

Each scorer extracts the final MCP tool call the agent made for a given
insight kind (``query-retention`` / ``query-funnel`` / ...) and grades it
against an expected shape or against the user prompt. Binary outputs only
— the LLM is asked yes/no.
"""
