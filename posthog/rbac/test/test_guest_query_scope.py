...
class TestUserIsGuest(BaseTest):
    def test_anonymous_user_is_not_guest(self) -> None:
        from django.contrib.auth.models import AnonymousUser

        self.assertFalse(user_is_guest(AnonymousUser()))

    def test_regular_member_is_not_guest(self) -> None:
        self.assertFalse(user_is_guest(self.user))

    def test_guest_membership_is_guest(self) -> None:
        guest_user = User.objects.create_user(email="guest2@example.com", first_name="G", password="password123")
        OrganizationMembership.objects.create(organization=self.organization, user=guest_user, is_guest=True)
        self.assertTrue(user_is_guest(guest_user))
